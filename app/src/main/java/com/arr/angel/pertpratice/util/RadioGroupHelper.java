...
public class RadioGroupHelper {
    ...
    public static void radioButtonLogic(Context context, FragmentManager fragmentManager,
                                        ...
                                        String answer, List<String> possibleAnswers, int nextQuestion) {
        switch (checkId) {
            case R.id.radioButton:
                if (answer.contentEquals(possibleAnswers.get(0))) {
                    Toast.makeText(context, "correct!", Toast.LENGTH_SHORT).show();
                    DialogCreations.showCorrectDialog(radioGroup, fragmentManager, nextQuestion);

                }else{
                    Toast.makeText(context, "incorrect!", Toast.LENGTH_SHORT).show();
                    DialogCreations.showIncorrectDialog(radioGroup, fragmentManager, nextQuestion);
